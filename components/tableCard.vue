<template>
  <v-card class="rounded-lg mx-1" :max-width="twidth" flat>
    <v-card-text>
      ...
      >
        <template v-slot:[`header.name`]="{ header }">
          <p class="accent headTable text-truncate font-weight-bold pb-n4 pa-0">
            {{ header.text }}
          </p>
        </template>
        ...
        <template v-slot:[`item.name`]="{ item }">
          <p class="font-weight-regular text-truncate">
            {{ item.name }}
          </p>
        </template>
          ...
          <v-menu bottom left rounded="lg" offset-y>
            ...
            <v-list dense>
              <v-list-item
                ...
                @click="
                  list.func == 'pause' && showPaused
                    ? pauseAccount(item)
                    : cantPause('Sorry, Account already paused')
                "
                :to="list.path ? `/companies/${item.id}` : `/companies`"
              >
